ndjson_query_character <-
    function(data, path, object_names, as, n_records, path_type)
{
...
ndjson_query_connection <-
    function(data, path, object_names, as, n_records, path_type, data_type)
{
    ...
    )
{
    ...
    if (any(c("file", "url") %in% data_type)) {
        ndjson_query_connection(
            data, path, object_names, as, n_records, path_type,
            data_type
        )
    } else {
        ndjson_query_character(
